...
public class EditOutfitModalController implements Initializable {
    ...
    public void setMemberId(Long memberId) {
        ...
        loadWardrobeForMember();
        loadCodiForMember();// 멤버 ID가 세팅되면 즉시 옷 불러오기
    }
    ...
    public void configure(List<Wardrobe> outfits, Codi codi, String scheduleName,
        ...
        if (outfits != null) {
            for (Wardrobe w : outfits) {
                selectedClothes.put(getCategoryName(w.getCategoryId()), w);
            }
        }
    ...
    private void refreshWardrobeSelection() {
        for (Node node : wardrobeItemPane.getChildren()) {
            ...
            boolean selected = selectedClothes.containsKey(clothing.getCategory())
                    && Objects.equals(selectedClothes.get(clothing.getCategory()).getId(), clothing.getId());
